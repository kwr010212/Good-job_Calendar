...
class RegisterPage(tk.Frame):
    ...
    def save(self):
        ...
        # 간단 포맷 검사(날짜)
        try:
            # datetime.fromisoformat expects YYYY-MM-DD or YYYY-MM-DDTHH:MM
            datetime_obj = core.datetime.fromisoformat(date_s)
            # if date only, okay; if user put time, it's acceptable but we still treat as date
        except Exception:
            # allow YYYY-MM-DD by trying date.fromisoformat
            try:
